...
class OwnerService:
	...
	def set_async_session(self, session: AsyncSession):
		...
		self.repository = SQLAlchemyRepository(session, LandOwner)

	async def update(self, owner_id, **values_set) -> LandOwner:
		try:
			owner = await self.repository.update_record(
				LandOwner.id == owner_id, **values_set
			)
			if not owner:
